    ...
    <section className="py-24 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 text-center space-y-8 relative z-10">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900 border border-purple-800 text-purple-300 text-xs font-black uppercase tracking-widest">
          ...
          <span>DOMLUVTE SI INDIVIDUÁLNÍ DEMO</span>
        </div>

        <h2 className="text-3xl sm:text-5xl lg:text-6xl font-black text-white tracking-tight leading-tight">
          Podívejte se, jak by SeePoint OS <br />
          <span className="bg-gradient-to-r from-purple-400 via-indigo-300 to-sky-400 bg-clip-text text-transparent">
            fungoval ve vaší firmě.
          </span>
        </h2>

        <p className="text-base sm:text-lg text-slate-300 font-medium max-w-2xl mx-auto leading-relaxed">
          Ukážeme vám systém na reálném workflow venkovní reklamy a společně projdeme možnosti převodu vašich současných dat a procesů.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
          <button
            type="button"
            onClick={() => {
              trackSaaSEvent('demo_cta_clicked', { source: 'final_cta' });
              onOpenDemoModal();
            }}
          ...
          <button
            type="button"
            onClick={() => {
              trackSaaSEvent('demo_cta_clicked', { source: 'final_cta_contact' });
              onOpenDemoModal();
            }}
        ...
        <div className="pt-4 flex items-center justify-center gap-2 text-xs text-slate-400 font-semibold">
          ...
          <span>Bezplatná konzultace · Bez závazků · Pomoc s migrací dat</span>
        </div>
      </div>
    </section>
  );
