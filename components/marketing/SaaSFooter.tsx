...
export function SaaSFooter({ onOpenDemoModal }: { onOpenDemoModal: () => void }) {
  return (
    <footer className="bg-slate-950 border-t border-slate-800 text-slate-300 text-sm py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          ...
          <div className="space-y-3">
            <h4 className="font-black text-white uppercase text-xs tracking-wider">Produkt</h4>
            <ul className="space-y-2.5 font-medium text-sm text-slate-300">
          ...
          {/* Column 2: Řešení */}
          <div className="space-y-3">
            <h4 className="font-black text-white uppercase text-xs tracking-wider">Řešení</h4>
            <ul className="space-y-2.5 font-medium text-sm text-slate-300">
          ...
          <div className="space-y-3">
            <h4 className="font-black text-white uppercase text-xs tracking-wider">Společnost</h4>
            <ul className="space-y-2.5 font-medium text-sm text-slate-300">
              <li><Link href="/login" className="hover:text-white transition">Přihlášení do aplikace</Link></li>
              <li><button type="button" onClick={onOpenDemoModal} className="hover:text-white transition text-left cursor-pointer">Domluvit ukázku</button></li>
              <li><a href="#faq" className="hover:text-white transition">Časté dotazy (FAQ)</a></li>
              <li className="pt-2 text-xs text-slate-400">© {new Date().getFullYear()} SeePoint OS</li>
            </ul>
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-slate-900 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-medium text-slate-400">
          ...
          <div className="flex items-center gap-6">
            <span className="hover:text-slate-200 transition cursor-pointer">Ochrana osobních údajů</span>
            <span className="hover:text-slate-200 transition cursor-pointer">Podmínky použití</span>
            <span className="hover:text-slate-200 transition cursor-pointer">Cookies</span>
          </div>
        </div>
      </div>
    </footer>
