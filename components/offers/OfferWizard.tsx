...
export function OfferWizard({ clients: initialClients, surfaces, initialOffer }: { clients: OfferClientOption[]; surfaces: OfferSurfaceOption[]; initialOffer?: OfferView }) {
  ...
  const [selectedPackageLimit, setSelectedPackageLimit] = useState<number | null>(null);
  ...
  const cityOptions = useMemo(() => [...new Set(surfaces.map((surface) => surface.carrier.city))].sort(), [surfaces]);
  const availableSurfaces = surfaces.filter((surface) => (selectedCities.size === 0 || selectedCities.has(surface.carrier.city)) && (!query || `${surface.carrier.code} ${surface.carrier.name} ${surface.name} ${surface.carrier.city} ${surface.carrier.locality ?? ''}`.toLocaleLowerCase('cs').includes(query.toLocaleLowerCase('cs'))));
  const selectedSurfaceIds = useMemo(() => new Set(items.map((item) => item.surfaceId)), [items]);
  const totals = useMemo(() => { let before = 0; let discount = 0; let subtotal = 0; for (const item of items) { const base = Math.round(cents(item.unitPrice) * cents(item.quantity) / 100); const reduction = Math.round(base * cents(item.discountPercent) / 10000) + cents(item.discountAmount); before += base; discount += reduction; subtotal += Math.max(0, base - reduction); } const tax = Math.round(subtotal * cents(taxRate) / 10000); return { before: formatCents(before), discount: formatCents(discount), subtotal: formatCents(subtotal), tax: formatCents(tax), total: formatCents(subtotal + tax) }; }, [items, taxRate]);
  ...
  const draftPayload = useMemo(() => ({ clientId, title, campaignName, contactPerson, contactEmail, contactPhone, campaignGoal, budget, validUntil, internalNote, clientMessage, taxRate, confirmNegotiation, items }), [budget, campaignGoal, campaignName, clientId, clientMessage, confirmNegotiation, contactEmail, contactPerson, contactPhone, internalNote, items, taxRate, title, validUntil]);
  ...
  function selectPackage(limit: number) { const chosen = [...availableSurfaces].sort((a, b) => Number(a.price) - Number(b.price)).slice(0, limit); setSelectedPackageLimit(limit); setItems(chosen.map((surface) => ({ surfaceId: surface.id, dateFrom, dateTo, quantity: '1', unit: 'plocha', unitPrice: surface.price || '0', discountPercent: '0', discountAmount: '0', note: '', groupLabel: surface.mediaType, customTitle: '', clientDescription: surface.carrier.description ?? '' }))); touch(); }
  ...
  return <div className="grid gap-6 lg:grid-cols-[240px_minmax(0,1fr)_300px]">
    ...
    <section className="card min-h-[420px] min-w-0">
      {step === 0 && <div><StepTitle title="Pro koho kampaň připravujeme?" subtitle="Vyberte klienta z CRM nebo založte nového." /><div className="grid gap-3 sm:grid-cols-2">{clients.map((client) => <button className={`rounded-xl border p-4 text-left transition ${clientId === client.id ? 'border-slate-950 bg-slate-50 ring-1 ring-slate-950' : 'border-slate-200 hover:border-slate-300'}`} key={client.id} onClick={() => selectClient(client.id)} type="button"><div className="flex items-center justify-between"><p className="font-semibold text-slate-950">{client.name}</p>{clientId === client.id && <Check size={18} />}</div><p className="mt-1 text-xs text-slate-500">{client.contactPerson || 'Kontaktní osoba neuvedena'}{client.companyId ? ` · IČO ${client.companyId}` : ''}</p></button>)}</div><button className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-sky-700" onClick={() => setShowClientForm((open) => !open)} type="button"><Plus size={15} /> Založit nového klienta</button>{showClientForm && <div className="mt-4 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2">{Object.entries(newClient).map(([key, current]) => <Field key={key} label={{ name: 'Název', contactPerson: 'Kontaktní osoba', email: 'E-mail', phone: 'Telefon', companyId: 'IČO', note: 'Poznámka' }[key] ?? key}><input className="input" onChange={(event) => setNewClient((value) => ({ ...value, [key]: event.target.value }))} value={current} /></Field>)}<button className="btn-primary sm:col-span-2" onClick={() => void createClient()} type="button">Vytvořit klienta</button></div>}<div className="mt-5 grid gap-3 sm:grid-cols-2"><Field label="Interní název nabídky"><input className="input" onChange={(event) => { setTitle(event.target.value); touch(); }} value={title} /></Field><Field label="Kontaktní osoba"><input className="input" onChange={(event) => { setContactPerson(event.target.value); touch(); }} value={contactPerson} /></Field><Field label="E-mail"><input className="input" onChange={(event) => { setContactEmail(event.target.value); touch(); }} type="email" value={contactEmail} /></Field><Field label="Telefon"><input className="input" onChange={(event) => { setContactPhone(event.target.value); touch(); }} value={contactPhone} /></Field></div></div>}
      ...
      {step === 6 && <div><StepTitle title="Výběr médií" subtitle="Vyberte konkrétní plochy podle preferovaného způsobu." /><div className="mb-5 flex flex-wrap gap-1 rounded-xl border border-slate-200 bg-white p-1 text-sm font-medium text-slate-600">{mediaModes.map(({ key, label, Icon }) => <button className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 transition ${mediaMode === key ? 'bg-slate-950 text-white' : 'hover:bg-slate-100'}`} key={key} onClick={() => setMediaMode(key)} type="button"><Icon size={16} />{label}</button>)}</div>
        {(mediaMode === 'map' || mediaMode === 'list') && <OfferSurfaceBrowser conflicts={conflicts} mode={mediaMode} onBulkToggle={bulkToggleSurfaces} onQueryChange={setQuery} onToggle={toggleSurface} query={query} selectedIds={selectedSurfaceIds} surfaces={surfaces.filter((surface) => selectedCities.size === 0 || selectedCities.has(surface.carrier.city))} />}
        {mediaMode === 'package' && <div className="grid gap-3 md:grid-cols-3">{[{ name: 'Lokální start', limit: 10, text: 'Kompaktní výběr nejdostupnějších ploch.' }, { name: 'Regionální mix', limit: 25, text: 'Vyvážený výběr napříč zvolenými městy.', top: true }, { name: 'Krajská dominance', limit: 45, text: 'Široké pokrytí dostupných lokalit.' }].map((pkg) => { const chosen = [...availableSurfaces].sort((a, b) => Number(a.price) - Number(b.price)).slice(0, pkg.limit); const price = chosen.reduce((sum, surface) => sum + Number(surface.price), 0); const selected = selectedPackageLimit === pkg.limit; return <button aria-pressed={selected} className={`flex flex-col rounded-xl border p-4 text-left transition ${selected ? 'border-slate-950 bg-slate-50 ring-1 ring-slate-950' : 'border-slate-200 hover:border-slate-950'}`} key={pkg.name} onClick={() => selectPackage(pkg.limit)} type="button"><div className="flex items-center justify-between gap-2"><p className="font-semibold text-slate-950">{pkg.name}</p>{selected ? <span className="inline-flex items-center gap-1 rounded-full bg-slate-950 px-2 py-1 text-xs font-semibold text-white"><Check aria-hidden="true" size={12} /> Vybráno</span> : pkg.top && <span className="rounded-full bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700">Top</span>}</div><p className="mt-1 text-xs text-slate-500">{pkg.text}</p><p className="mt-3 text-2xl font-semibold text-slate-950">{money(price)}</p><p className="text-xs text-slate-500">{chosen.length} reálných ploch</p></button>; })}</div>}
        ...
        {items.length > 0 && <div className="mt-6 space-y-3 border-t border-slate-200 pt-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2"><button className="rounded-xl border border-slate-300 px-4 py-3 text-sm font-semibold" disabled={saving} onClick={() => void save('draft')} type="button">{saving ? 'Ukládám…' : 'Uložit a pokračovat na plánování'}</button><button className="rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white disabled:opacity-40" disabled={saving || !checked || conflicts.some((conflict) => conflict.severity === 'block') || (conflicts.some((conflict) => conflict.severity === 'warning') && !confirmNegotiation)} onClick={() => void save('send')} type="button">{saving ? 'Odesílám…' : 'Uložit a otevřít náhled'}</button></div>
        </div>}
      </div>}
