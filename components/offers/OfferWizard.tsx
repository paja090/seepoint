...
import type { OfferClientOption, OfferPriceRuleOption, OfferSurfaceOption, OfferView } from '@/lib/offers/view-model';
...
export function OfferWizard({ clients: initialClients, surfaces, priceRules, initialOffer }: { clients: OfferClientOption[]; surfaces: OfferSurfaceOption[]; priceRules: OfferPriceRuleOption[]; initialOffer?: OfferView }) {
  ...
  const draftPayload = useMemo(() => ({ clientId, title, campaignName, contactPerson, contactEmail, contactPhone, campaignGoal, budget, validUntil, internalNote, clientMessage, taxRate, confirmNegotiation, items, chargeSelections }), [budget, campaignGoal, campaignName, chargeSelections, clientId, clientMessage, confirmNegotiation, contactEmail, contactPerson, contactPhone, internalNote, items, taxRate, title, validUntil]);
