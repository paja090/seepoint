...
export function OfferWizard({ clients: initialClients, surfaces, priceRules, mediaPackages, initialOffer }: { clients: OfferClientOption[]; surfaces: OfferSurfaceOption[]; priceRules: OfferPriceRuleOption[]; mediaPackages: MediaPackageOption[]; initialOffer?: OfferView }) {
  ...
  const draftPayload = useMemo(() => ({ clientId, title, campaignName, contactPerson, contactEmail, contactPhone, campaignGoal, budget, validUntil, internalNote, clientMessage, taxRate, confirmNegotiation, packageId: selectedPackageId || undefined, items, chargeSelections }), [budget, campaignGoal, campaignName, chargeSelections, clientId, clientMessage, confirmNegotiation, contactEmail, contactPerson, contactPhone, internalNote, items, selectedPackageId, taxRate, title, validUntil]);
