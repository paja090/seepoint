...
const stages = [
  ...
  { key: 'approval', label: 'Interní schválení' },
  { key: 'preview', label: 'Nabídka klientovi' },
          ...
          return (
            <li className="flex items-center gap-1" key={stage.key}>
              <Link
                ...
                className={`group flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium transition ${active ? 'bg-slate-950 text-white' : done ? 'text-emerald-700 hover:bg-emerald-50' : 'text-slate-500 hover:bg-slate-100'}`}
                href={href(stage.key)}
              >
                <span className={`grid size-6 shrink-0 place-items-center rounded-full text-xs font-semibold ${active ? 'bg-white text-slate-950' : done ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-600'}`}>
