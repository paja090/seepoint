...
import { offerMissingAssets, offerReadinessChecks, type OfferConflictView, type OfferReadinessStatus } from '@/lib/offers/workflow';
import { OfferProcessStepper } from './OfferProcessStepper';
import { OfferSendControl } from './OfferSendControl';

const statusMeta: Record<OfferReadinessStatus, { icon: React.ReactNode; ring: string; bg: string; text: string; label: string }> = {
...
export function OfferApproval({ offer, conflicts }: { offer: OfferView; conflicts: OfferConflictView[] }) {
  ...
  const checks = offerReadinessChecks(offer, conflicts);
        ...
        <div className="space-y-6">
          <section className="card">
            <div className="mb-4 flex items-center gap-2">
              <ShieldCheck aria-hidden="true" className="text-slate-500" size={18} />
              <h2 className="text-base font-semibold text-slate-950">Kontrola před odesláním</h2>
              <span className={`ml-auto rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${errorCount ? 'bg-red-50 text-red-700 ring-red-200' : 'bg-emerald-50 text-emerald-700 ring-emerald-200'}`}>
                {checks.filter((check) => check.status === 'ok').length}/{checks.length} splněno
              </span>
            </div>
            <ul className="space-y-2.5">
              {checks.map((item) => {
                const meta = statusMeta[item.status];
                return <li className={`flex items-start gap-3 rounded-xl p-3 ring-1 ${meta.ring} ${meta.bg}`} key={item.id}><span className={`mt-0.5 ${meta.text}`}>{meta.icon}</span><div><div className="flex items-center gap-2"><p className="font-semibold text-slate-900">{item.label}</p><span className={`text-xs font-semibold ${meta.text}`}>· {meta.label}</span></div><p className="mt-0.5 text-sm text-slate-600">{item.detail}</p></div></li>;
              })}
            </ul>
          </section>
