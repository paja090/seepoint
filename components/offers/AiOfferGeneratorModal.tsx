...
type Preview = {
  ...
  items: Array<{ surfaceId: string; carrierCode: string; title: string; mediaType: string; city: string; latitude: number | null; longitude: number | null; catalogPrice: number | null; finalPrice: number | null; score: number; reasons: string[]; distanceMeters?: number; routePolyline?: string; arrowDirection?: string }>;
    ...
    <div className="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-slate-700 bg-slate-900 text-slate-100 shadow-2xl">
      ...
      <div className="flex-1 space-y-6 overflow-y-auto p-5">
        ...
        {preview && <section className="space-y-4 rounded-2xl border border-emerald-700 bg-slate-950 p-4"><div className="flex flex-wrap justify-between gap-3"><div><h3 className="text-lg font-black">Návrh nabídky: {typeLabels[preview.offerType]}</h3><p className="text-xs text-slate-400">{preview.client.name} · {segmentLabels[preview.client.pricingSegment]} · {preview.city || 'bez lokality'} · {preview.dateFrom} – {preview.dateTo}</p></div><div className="text-right"><div className="text-xs text-slate-400">Ceníková cena</div><strong className="text-xl">{preview.catalogTotal === null ? 'Nelze určit' : `${preview.catalogTotal.toLocaleString('cs-CZ')} Kč`}</strong>{preview.budgetDifference != null && <div className={preview.budgetDifference > 0 ? 'text-rose-400' : 'text-emerald-400'}>{preview.budgetDifference > 0 ? '+' : ''}{preview.budgetDifference.toLocaleString('cs-CZ')} Kč vůči rozpočtu</div>}</div></div>
          ...
          {preview.offerType === 'NAVIGATION' && preview.target && <GoogleNavigationOfferMap target={{ latitude: preview.target.latitude, longitude: preview.target.longitude, label: preview.target.name, address: preview.target.address }} points={preview.items.filter((item) => item.latitude != null && item.longitude != null).map((item) => ({ id: item.surfaceId, label: item.carrierCode, latitude: item.latitude!, longitude: item.longitude!, calculatedDistanceMeters: item.distanceMeters, routePolyline: item.routePolyline, arrowDirectionEnum: item.arrowDirection }))} mode="point" onMapClick={() => undefined} onPointMove={() => undefined} onTargetSelect={() => undefined} />}
          <div className="max-h-64 space-y-2 overflow-y-auto">{preview.items.map((item, index) => <div className="rounded-xl border border-slate-800 p-3" key={item.surfaceId}><div className="flex justify-between gap-2"><strong>{index + 1}. {item.carrierCode} · {item.title}</strong><span>{item.finalPrice == null ? '⚠️ bez ceny' : `${item.finalPrice.toLocaleString('cs-CZ')} Kč`}</span></div><p className="mt-1 text-xs text-slate-400">Skóre {item.score}/100 · {item.distanceMeters ? `${item.distanceMeters} m od cíle · ` : ''}{item.reasons.join(' ')}</p><button className="mt-2 text-xs font-bold text-rose-300" onClick={() => setPreview((current) => current ? { ...current, items: current.items.filter((candidate) => candidate.surfaceId !== item.surfaceId) } : current)} type="button">Odebrat bod / plochu</button></div>)}</div>
