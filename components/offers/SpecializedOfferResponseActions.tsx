...
export function SpecializedOfferResponseActions({ status, token, isLocationSelection }: { status: string; token: string; isLocationSelection?: boolean }) {
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            <button className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white hover:bg-emerald-700 cursor-pointer" onClick={() => setAction('approve')} type="button">
              ...
              {isLocationSelection ? 'Potvrdit výběr lokalit' : 'Schválit nabídku'}
            </button>
            <button className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-sky-200 bg-white px-4 text-sm font-bold text-sky-800 hover:bg-sky-50 cursor-pointer" onClick={() => setAction('revision')} type="button">
              <PencilLine size={18} />Požádat o úpravu
            </button>
            <button className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 hover:bg-slate-50 cursor-pointer" onClick={() => setAction('question')} type="button">
