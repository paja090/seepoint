...
import { formatWorkPrice, workPriorityLabels, workPriorityStyles, workStatusLabels, workStatusStyles, workTypeLabels } from '@/lib/work';
            ...
            return <section className={`min-h-64 rounded-2xl border p-3 ${isToday ? 'border-sky-400 bg-sky-50' : 'border-slate-200 bg-slate-50'}`} key={day.toISOString()} aria-label={day.toLocaleDateString('cs-CZ', { weekday: 'long', day: 'numeric', month: 'numeric' })}>
              ...
              <div className="space-y-3">{dayOrders.length === 0 ? <p className="text-xs text-slate-400">Bez práce</p> : dayOrders.map((order) => {
                ...
                return <article className={`rounded-xl border bg-white p-3 shadow-sm ${overdue ? 'border-red-400 ring-2 ring-red-100' : order.priority === 'URGENT' ? 'border-red-300 bg-red-50' : 'border-slate-200'}`} key={order.id}>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2"><button className={`rounded-lg px-2 py-1 text-xs font-medium ${order.status === 'IN_PROGRESS' ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-600'}`} disabled={savingId === order.id} onClick={() => void updateOrder(order, { status: 'IN_PROGRESS' })} type="button">Probíhá</button><button className={`rounded-lg px-2 py-1 text-xs font-medium ${order.status === 'DONE' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600'}`} disabled={savingId === order.id} onClick={() => void updateOrder(order, { status: 'DONE' })} type="button">Hotovo</button><button aria-pressed={order.ftdSent} className={`col-span-2 rounded-lg px-2 py-1 text-xs font-medium ${order.ftdSent ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'}`} disabled={savingId === order.id} onClick={() => void updateOrder(order, { ftdSent: !order.ftdSent })} type="button">Fotodokumentace {order.ftdSent ? '✓' : '—'}</button></div>
