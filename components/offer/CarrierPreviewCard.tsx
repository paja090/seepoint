...
import type { MockCarrier } from '@/lib/mock-offer-data';
import { MEDIA_TYPE_META, TONE_CLASSES } from '@/lib/mock-offer-data';

const statusMeta: Record<MockCarrier['status'], { label: string; className: string }> = {
...
export function CarrierPreviewCard({
  ...
  carrier: MockCarrier;
  onOpen: (carrier: MockCarrier) => void;
}) {
  const meta = MEDIA_TYPE_META[carrier.mediaType];
  const tone = TONE_CLASSES[meta.tone];
  const status = statusMeta[carrier.status];
