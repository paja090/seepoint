 ...
 */
export async function registerResendDomain(domain: string, ownedProviderDomainId?: string): Promise<ResendDomainResponse> {
  ...
  if (!res.ok) {
    ...
    if (/registered already/i.test(data?.message || '') || /already exists/i.test(data?.message || '')) {
      try {
        ...
        if (existing && existing.id === ownedProviderDomainId) {
