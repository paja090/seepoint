...
export async function createOfferPdf(offer: ProposalOffer, clientLogoDataUrl?: string): Promise<Buffer> {
  ...
  const carrierRows = offer.carriers.slice(0, 18).map((carrier, index) => [
    ...
    content: [
      {
        table: {
          ...
          body: [[
            { stack: [
              { text: 'NABÍDKA REKLAMNÍ KAMPANĚ', color: BLUE, bold: true, fontSize: 9 },
      ...
      { text: offer.intro, fontSize: 10, color: MUTED, margin: [4, 0, 4, 16] },
      {
        columns: [
          ...
          { stack: [{ text: 'ROZSAH', style: 'eyebrow' }, { text: `${offer.stats.carriers} ploch`, bold: true }, { text: `${offer.stats.locations} lokalit · ${offer.stats.mediaTypes} typů médií`, color: MUTED, fontSize: 8 }] },
          { stack: [{ text: 'PLATNOST', style: 'eyebrow' }, { text: offer.validUntil, bold: true }, { text: 'do tohoto data', color: MUTED, fontSize: 8 }] },
        ],
        columnGap: 10,
        margin: [0, 0, 0, 20],
      },
      { text: 'Vybrané reklamní plochy', style: 'heading' },
      {
        table: {
          headerRows: 1,
          widths: [18, '*', 105, 78],
      ...
      ...(offer.carriers.length > 18 ? [{ text: `Dalších ${offer.carriers.length - 18} ploch je uvedeno v interaktivní nabídce.`, fontSize: 8, color: MUTED, margin: [0, 0, 0, 12] }] : []),
