...
export async function listMediaPackages() {
  return prisma.mediaPackage.findMany({
    include: {
      rules: {
        include: {
          carrierTypeRef: {
...
export async function createMediaPackage(user: CurrentUser, raw: unknown) {
  ...
  const rules = rows.map((rawRule, index) => {
    if (!rawRule || typeof rawRule !== 'object') throw new OfferValidationError(`Pravidlo ${index + 1} není platné.`);
    const rule = rawRule as Record<string, unknown>;
    const mediaType = text(rule.mediaType);
    const carrierTypeId = text(rule.carrierTypeId) || null;
    const quantity = Number(rule.quantity);
    if (!Number.isInteger(quantity) || quantity < 1) throw new OfferValidationError(`Pravidlo ${index + 1} nemá platný počet.`);
    if (!carrierTypeId && (!mediaType || !Object.values(MediaType).includes(mediaType as MediaType))) {
      throw new OfferValidationError(`Pravidlo ${index + 1} nemá platný typ média.`);
    }
    return {
      mediaType: (mediaType as MediaType) || MediaType.BILLBOARD,
      carrierTypeId,
      city: text(rule.city) || null,
      locality: text(rule.locality) || null,
      quantity,
      sortOrder: index,
    };
  });
  return prisma.mediaPackage.create({
    data: {
      ...
      standardPrice: nullableDecimal(input.standardPrice, 'Standardní cena'),
      packagePrice: nullableDecimal(input.packagePrice, 'Balíčková cena'),
      defaultDuration: text(input.defaultDuration) ? Number(input.defaultDuration) : null,
      rules: { create: rules },
    },
    include: {
      rules: {
...
export async function archiveMediaPackage(user: CurrentUser, id: string) {
  assertAdmin(user);
  return prisma.mediaPackage.update({
    where: { id },
    data: { active: false },
    include: { rules: true },
  });
