...
export function serializeOffer(row: OfferRow, options: { publicToken?: string; publicView?: boolean } = {}) {
  ...
  const serialized = {
    ...
    isNoPriceConcept: row.isNoPriceConcept ?? false,
    campaignStrategy: row.campaignStrategy ?? null,
    campaignPhases: row.campaignPhases ?? null,
    currency: row.currency,
    taxRate: publicView && row.isNoPriceConcept ? undefined : value(row.taxRate),
    subtotalBeforeDiscount: publicView && row.isNoPriceConcept ? undefined : row.items.reduce((sum, item) => sum.add((item.quantity ?? new Prisma.Decimal(1)).mul(item.unitPrice ?? item.price ?? 0)), new Prisma.Decimal(0)).add(row.charges.reduce((sum, charge) => sum.add(charge.subtotal), new Prisma.Decimal(0))).toFixed(2),
    subtotal: publicView && row.isNoPriceConcept ? undefined : value(row.subtotal ?? row.totalPrice),
    discountAmount: publicView && row.isNoPriceConcept ? undefined : value(row.discountAmount),
    taxAmount: publicView && row.isNoPriceConcept ? undefined : value(row.taxAmount),
    totalWithTax: publicView && row.isNoPriceConcept ? undefined : value(row.totalWithTax ?? row.totalPrice),
    ...
    items: row.items.map((item) => {
      ...
      return {
        ...
        unit: item.unit,
        unitPrice: publicView && row.isNoPriceConcept ? undefined : value(item.unitPrice ?? item.price),
        discountPercent: publicView && row.isNoPriceConcept ? undefined : value(item.discountPercent),
        discountAmount: publicView && row.isNoPriceConcept ? undefined : value(item.discountAmount),
        fixedDiscountAmount: publicView && row.isNoPriceConcept ? undefined : recoverFixedDiscount(
          value(item.quantity) ?? '1',
          value(item.unitPrice ?? item.price) ?? '0',
          value(item.discountPercent) ?? '0',
          value(item.discountAmount),
        ),
        subtotal: publicView && row.isNoPriceConcept ? undefined : value(item.subtotal ?? item.price),
    ...
    charges: publicView && row.isNoPriceConcept ? [] : row.charges.map((charge) => ({
