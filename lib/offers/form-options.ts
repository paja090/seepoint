...
import type { OfferClientOption, OfferPriceRuleOption, OfferSurfaceOption } from './view-model';

export async function getOfferFormOptions() {
  const [clients, surfaces, priceRules] = await Promise.all([
  ...
  return { clients: clientOptions, surfaces: surfaceOptions, priceRules: pricing };
}
