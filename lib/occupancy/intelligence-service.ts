...
export async function runOccupancyAudit(
  ...
  // Fetch all existing OPEN or REVIEWED insights for this organization
    ...
    if (existing) {
      ...
      await prisma.occupancyInsight.update({
        ...
        data: {
          ...
          metadata: finding.metadata ? (finding.metadata as Prisma.InputJsonValue) : undefined,
          detectedAt: now,
        },
      });
    } else {
      // Create new finding
      await prisma.occupancyInsight.create({
        data: {
          organizationId,
          type: finding.type,
          severity: finding.severity,
  ...
  // Auto-resolve insights whose issue is no longer present
