...
function serializeOfferItem(item: Prisma.OfferItemGetPayload<{ include: { surface: { include: { carrier: true } } } }>): OfferItem { return { id: item.id, offerId: item.offerId, surfaceId: item.surfaceId, surface: { id: item.surface.id, name: item.surface.name, mediaType: item.surface.mediaType, status: item.surface.status, carrier: { id: item.surface.carrier.id, name: item.surface.carrier.name, code: item.surface.carrier.code, city: item.surface.carrier.city, locality: item.surface.carrier.locality ?? item.surface.carrier.cadastralArea ?? undefined, address: item.surface.carrier.address ?? item.surface.carrier.street ?? undefined } }, dateFrom: dateOnly(item.dateFrom)!, dateTo: dateOnly(item.dateTo)!, price: item.price?.toNumber(), note: item.note ?? undefined }; }
function serializeOffer(offer: Prisma.OfferGetPayload<{ include: { client: true, items: { include: { surface: { include: { carrier: true } } } } } }>): Offer { return { id: offer.id, clientId: offer.clientId, client: { id: offer.client.id, name: offer.client.name }, title: offer.title, status: offer.status, validUntil: dateOnly(offer.validUntil), note: offer.note ?? undefined, totalPrice: offer.totalPrice?.toNumber(), createdBy: offer.createdBy ?? undefined, createdAt: offer.createdAt.toISOString(), updatedAt: offer.updatedAt.toISOString(), items: offer.items.map(serializeOfferItem) }; }
export async function getOffers(): Promise<Offer[]> { const offers = await prisma.offer.findMany({ include: { client: true, items: { include: { surface: { include: { carrier: true } } }, orderBy: { dateFrom: 'asc' } } }, orderBy: [{ updatedAt: 'desc' }], take: 200 }); return offers.map(serializeOffer); }
export type OfferDraftInput = { clientId: string; title: string; status?: 'DRAFT' | 'SENT'; validUntil?: string; note?: string; createdBy?: string; items: Array<{ surfaceId: string; dateFrom: string; dateTo: string; price?: number; note?: string }> };
export async function createOfferDraft(input: OfferDraftInput) { const conflicts = (await Promise.all(input.items.map((item) => checkOccupancyConflicts([item.surfaceId], item.dateFrom, item.dateTo)))).flat(); const occupiedConflicts = conflicts.filter((conflict) => conflict.status === 'OCCUPIED'); if (occupiedConflicts.length > 0) { const error = new Error('Nabidku nelze ulozit, protoze nektere plochy jsou ve zvolenem terminu obsazene.'); (error as Error & { conflicts?: OccupancyConflict[] }).conflicts = occupiedConflicts; throw error; } const totalPrice = input.items.reduce((sum, item) => sum + (Number(item.price) || 0), 0); const offer = await prisma.offer.create({ data: { clientId: input.clientId, title: input.title.trim() || 'Nova nabidka', status: input.status ?? 'DRAFT', validUntil: input.validUntil ? new Date(input.validUntil) : null, note: input.note?.trim() || null, createdBy: input.createdBy?.trim() || null, totalPrice, items: { create: input.items.map((item) => ({ surfaceId: item.surfaceId, dateFrom: new Date(item.dateFrom), dateTo: new Date(item.dateTo), price: item.price ?? null, note: item.note?.trim() || null })) } }, include: { client: true, items: { include: { surface: { include: { carrier: true } } } } } }); return { offer: serializeOffer(offer), conflicts }; }
