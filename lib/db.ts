...
import type { Carrier, CarrierType, GpsStatus, MediaType, Occupancy, Offer, OfferItem, OccupancyStatus, Surface, SurfaceStatus } from './types';
...
function serializeOccupancy(occupancy: OccupancyRow): Occupancy {
  return {
    id: occupancy.id,
    surfaceId: occupancy.surfaceId,
    clientId: occupancy.clientId ?? undefined,
    clientName: occupancy.clientName,
    campaignName: occupancy.campaignName,
    dateFrom: dateOnly(occupancy.dateFrom)!,
    dateTo: dateOnly(occupancy.dateTo)!,
    status: occupancy.status,
    price: occupancy.price?.toNumber(),
    note: occupancy.note ?? undefined,
    createdBy: occupancy.createdBy ?? undefined,
    updatedBy: occupancy.updatedBy ?? undefined,
    reservedUntil: dateOnly(occupancy.reservedUntil),
    offerId: occupancy.offerId ?? undefined,
...
export async function getCarrierFilterOptions(): Promise<CarrierFilterOptions> {
  ...
  return {
    ...
    importBatches: importBatches.map((batch) => ({ id: batch.id, label: `${batch.fileName} (${batch.createdAt.toLocaleDateString('cs-CZ')})` })),
  };
}
...
export type OccupancyFilters = {
  ...
  city?: string;
  locality?: string;
  status?: OccupancyStatus | SurfaceStatus;
  dateFrom?: string;
  dateTo?: string;
};

export type OccupancyRowView = {
  occupancy: Occupancy | null;
  surface: Pick<Surface, 'id' | 'name' | 'mediaType' | 'status' | 'price'>;
  carrier: Pick<Carrier, 'id' | 'name' | 'code' | 'city' | 'locality' | 'address'>;
  currentClient?: Pick<NonNullable<Surface['currentClient']>, 'id' | 'name'>;
};
...
function overlaps(dateFrom: Date, dateTo: Date) {
  return { dateFrom: { lte: dateTo }, dateTo: { gte: dateFrom } };
}
...
export async function getOccupancyOverview(filters: OccupancyFilters = {}) {
  ...
  const carrierWhere: Prisma.AdvertisingCarrierWhereInput = { archivedAt: null };
  ...
  const surfaces = await prisma.advertisingSurface.findMany({
    where: { ...surfaceWhere, carrier: carrierWhere, ...(Object.keys(occupancyWhere).length ? { occupancies: { some: occupancyWhere } } : {}) },
    include: { currentClient: true, carrier: true, occupancies: { where: Object.keys(occupancyWhere).length ? occupancyWhere : undefined, orderBy: [{ dateTo: 'asc' }, { dateFrom: 'asc' }] } },
    orderBy: [{ carrier: { city: 'asc' } }, { carrier: { name: 'asc' } }, { name: 'asc' }],
    take: 1000,
  });

  const rows = surfaces.flatMap((surface): OccupancyRowView[] => {
    const base = {
      surface: { id: surface.id, name: surface.name, mediaType: surface.mediaType, status: surface.status, price: surface.price?.toNumber() },
      carrier: { id: surface.carrier.id, name: surface.carrier.name, code: surface.carrier.code, city: surface.carrier.city, locality: surface.carrier.locality ?? surface.carrier.cadastralArea ?? undefined, address: surface.carrier.address ?? surface.carrier.street ?? undefined },
      currentClient: surface.currentClient ? { id: surface.currentClient.id, name: surface.currentClient.name } : undefined,
    };
    if (surface.occupancies.length === 0) return [{ ...base, occupancy: null }];
    return surface.occupancies.map((occupancy) => ({ ...base, occupancy: serializeOccupancy(occupancy) }));
  });

  const today = new Date();
  const in7 = new Date(today); in7.setDate(today.getDate() + 7);
  const in30 = new Date(today); in30.setDate(today.getDate() + 30);
  const activeRows = rows.filter((row) => row.occupancy && ['OCCUPIED', 'RESERVED', 'NEGOTIATION'].includes(row.occupancy.status));

  return {
    rows,
    ending7: activeRows.filter((row) => row.occupancy && parseDateInput(row.occupancy.dateTo)! <= in7),
    ending30: activeRows.filter((row) => row.occupancy && parseDateInput(row.occupancy.dateTo)! <= in30),
    available: rows.filter((row) => !row.occupancy && row.surface.status === 'AVAILABLE'),
    reserved: rows.filter((row) => row.occupancy?.status === 'RESERVED'),
    negotiation: rows.filter((row) => row.occupancy?.status === 'NEGOTIATION'),
...
export async function checkOccupancyConflicts(surfaceIds: string[], dateFromInput: string, dateToInput: string, ignoreOccupancyId?: string): Promise<OccupancyConflict[]> {
  ...
  const dateTo = parseDateInput(dateToInput);
  if (!dateFrom || !dateTo || dateFrom > dateTo || surfaceIds.length === 0) return [];
  const conflicts = await prisma.occupancy.findMany({
    where: { surfaceId: { in: surfaceIds }, id: ignoreOccupancyId ? { not: ignoreOccupancyId } : undefined, status: { in: ['OCCUPIED', 'RESERVED', 'NEGOTIATION'] }, ...overlaps(dateFrom, dateTo) },
    include: { surface: { include: { carrier: true } } },
    orderBy: { dateFrom: 'asc' },
  });
  return conflicts.map((conflict) => ({
    surfaceId: conflict.surfaceId,
    surfaceName: conflict.surface.name,
    carrierName: conflict.surface.carrier.name,
    carrierCode: conflict.surface.carrier.code,
    status: conflict.status,
    clientName: conflict.clientName,
    campaignName: conflict.campaignName,
    dateFrom: dateOnly(conflict.dateFrom)!,
    dateTo: dateOnly(conflict.dateTo)!,
    severity: conflict.status === 'OCCUPIED' || conflict.status === 'RESERVED' ? 'block' : 'warning',
  }));
}

export function hasBlockingConflict(conflicts: OccupancyConflict[]) {
  return conflicts.some((conflict) => conflict.severity === 'block');
}

function serializeOfferItem(item: Prisma.OfferItemGetPayload<{ include: { surface: { include: { carrier: true } } } }>): OfferItem {
  return {
    id: item.id,
    offerId: item.offerId,
    surfaceId: item.surfaceId,
    surface: { id: item.surface.id, name: item.surface.name, mediaType: item.surface.mediaType, status: item.surface.status, carrier: { id: item.surface.carrier.id, name: item.surface.carrier.name, code: item.surface.carrier.code, city: item.surface.carrier.city, locality: item.surface.carrier.locality ?? item.surface.carrier.cadastralArea ?? undefined, address: item.surface.carrier.address ?? item.surface.carrier.street ?? undefined } },
...
function serializeOffer(offer: Prisma.OfferGetPayload<{ include: { client: true, items: { include: { surface: { include: { carrier: true } } } } } }>): Offer {
  return { id: offer.id, clientId: offer.clientId, client: { id: offer.client.id, name: offer.client.name }, title: offer.title, status: offer.status, validUntil: dateOnly(offer.validUntil), note: offer.note ?? undefined, totalPrice: offer.totalPrice?.toNumber(), createdBy: offer.createdBy ?? undefined, createdAt: offer.createdAt.toISOString(), updatedAt: offer.updatedAt.toISOString(), items: offer.items.map(serializeOfferItem) };
}

export async function getOffers(): Promise<Offer[]> {
  const offers = await prisma.offer.findMany({ include: { client: true, items: { include: { surface: { include: { carrier: true } } }, orderBy: { dateFrom: 'asc' } } }, orderBy: [{ updatedAt: 'desc' }], take: 200 });
...
export type OfferDraftInput = {
  ...
  note?: string;
  createdBy?: string;
  items: Array<{ surfaceId: string; dateFrom: string; dateTo: string; price?: number; note?: string }>;
};

export async function createOfferDraft(input: OfferDraftInput) {
  const conflicts = (await Promise.all(input.items.map((item) => checkOccupancyConflicts([item.surfaceId], item.dateFrom, item.dateTo)))).flat();
  const occupiedConflicts = conflicts.filter((conflict) => conflict.status === 'OCCUPIED');
  if (occupiedConflicts.length > 0) {
    const error = new Error('Nabidku nelze ulozit, protoze nektere plochy jsou ve zvolenem terminu obsazene.');
    (error as Error & { conflicts?: OccupancyConflict[] }).conflicts = occupiedConflicts;
    throw error;
  }
  const totalPrice = input.items.reduce((sum, item) => sum + (Number(item.price) || 0), 0);
  const offer = await prisma.offer.create({
    data: { clientId: input.clientId, title: input.title.trim() || 'Nova nabidka', status: input.status ?? 'DRAFT', validUntil: input.validUntil ? new Date(input.validUntil) : null, note: input.note?.trim() || null, createdBy: input.createdBy?.trim() || null, totalPrice, items: { create: input.items.map((item) => ({ surfaceId: item.surfaceId, dateFrom: new Date(item.dateFrom), dateTo: new Date(item.dateTo), price: item.price ?? null, note: item.note?.trim() || null })) } },
    include: { client: true, items: { include: { surface: { include: { carrier: true } } } } },
  });
  return { offer: serializeOffer(offer), conflicts };
}
...
export async function upsertOccupancy(input: Partial<Occupancy> & { surfaceId: string }): Promise<Occupancy> {
  ...
  const data = { surfaceId: input.surfaceId, clientId: input.clientId ?? existing?.clientId ?? null, clientName: input.clientName ?? existing?.clientName ?? 'Klient', campaignName: input.campaignName ?? existing?.campaignName ?? 'Kampan', dateFrom: input.dateFrom ? new Date(input.dateFrom) : existing?.dateFrom ?? new Date(), dateTo: input.dateTo ? new Date(input.dateTo) : existing?.dateTo ?? new Date(), status: input.status ?? existing?.status ?? 'RESERVED', price: input.price ?? existing?.price ?? null, note: input.note ?? existing?.note ?? null, createdBy: input.createdBy ?? existing?.createdBy ?? null, updatedBy: input.updatedBy ?? existing?.updatedBy ?? null, reservedUntil: input.reservedUntil ? new Date(input.reservedUntil) : existing?.reservedUntil ?? null, offerId: input.offerId ?? existing?.offerId ?? null };
  const saved = existing ? await prisma.occupancy.update({ where: { id: existing.id }, data }) : await prisma.occupancy.create({ data: { ...data, id: input.id } });
  const activeSurfaceStatuses: Partial<Record<OccupancyStatus, SurfaceStatus>> = { AVAILABLE: 'AVAILABLE', NEGOTIATION: 'NEGOTIATION', RESERVED: 'RESERVED', OCCUPIED: 'OCCUPIED', OUT_OF_SERVICE: 'OUT_OF_SERVICE' };
  const surfaceStatus = activeSurfaceStatuses[saved.status] ?? 'AVAILABLE';
  await prisma.advertisingSurface.update({ where: { id: saved.surfaceId }, data: { status: surfaceStatus, currentClientId: ['NEGOTIATION', 'RESERVED', 'OCCUPIED'].includes(saved.status) ? saved.clientId : null } });
...
export async function updateOccupancyAction(id: string, action: OccupancyAction, input: { dateTo?: string; updatedBy?: string } = {}) {
  return prisma.$transaction(async (transaction) => {
    ...
    await transaction.advertisingSurface.update({ where: { id: existing.surfaceId }, data: { status: 'AVAILABLE', currentClientId: null } });
    return serializeOccupancy(occupancy);
  });
}
