...
- Final scheduled-handler test on preview 6afbbaa: test profile was disabled, so the first call correctly returned SKIPPED_DISABLED. After explicitly enabling only test organization A, authenticated POST /api/cron/sales-radar completed in 24.2 seconds: exactly 1 organization scanned, 7 created, 1 duplicate, no deferred organizations, run cmtu2jiqh0001ic044hvo29uv.
