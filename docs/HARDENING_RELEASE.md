...
Main 7f1e98f doplňuje alternativu k ručnímu přiřazení dosud nespárované domény: administrátor může explicitně zadat vlastní Resend klíč, který poskytovatel ověří při načtení domén. Klíč ze systémového env tuto možnost sám o sobě neposkytuje. Ani explicitní klíč nesmí obejít unikátní vlastnictví domény uložené v DB. Token keyring a pořadí migrací zůstávají stejné.

Každý nový běh integračních mutačních testů začněte novým `e2e/seed.ts`: vytváří novou sadu syntetických organizací a `database-check.ts` jejich workflow úmyslně dokončí. Pro přímé spuštění použijte `node --env-file=.env.local --import tsx e2e/seed.ts`, potom `node --env-file=.env.local --env-file=.env.e2e.local --import tsx e2e/database-check.ts`. Browser sada načítá `.env.e2e.local` přes Playwright config; podporuje také instalovaný Chrome nastavením E2E_BROWSER_CHANNEL=chrome.
