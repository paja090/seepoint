  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-7xl space-y-6">
        <PageHeader
          title="Plán práce"
          description="Instalace, opravy, kontroly a převozy na jednom místě. Při uložení zakázky se automaticky vytvoří interní úkoly pro zaměstnance."
        />
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
          <StatCard label="Dnes" value={todayCount} />
          <StatCard label="Příštích 7 dní" value={weekCount} />
          <StatCard label="Otevřené úkoly" value={openCount} />
          <StatCard label="Interní úkoly" value={internalTaskCount} />
          <StatCard label="Urgentní" value={urgentCount} tone="red" />
          <StatCard label="Připravené k fakturaci" value={billingOrders.length} tone="green" />
        </section>
        ...
        <section className="space-y-3">
          <div className="flex items-end justify-between gap-4"><div><h2 className="text-2xl font-bold text-slate-950">Nejbližší pracovní úkoly</h2><p className="text-sm text-slate-500">Řazeno podle data provedení. Kompletní plán zůstává v týdenním rozpisu.</p></div><span className="text-sm font-medium text-slate-500">{listOrders.length} z {orders.length} úkolů</span></div>
          {orders.length === 0 ? <EmptyState title="Zatím zde není žádný pracovní úkol." description="První úkol vytvořte ve formuláři výše. Původní tabulka zůstává beze změny." /> : listOrders.map((order) => {
            const isOverdue = order.deadlineAt && order.deadlineAt < now && !['DONE', 'CANCELLED'].includes(order.status);
            return <Link className={`card block transition hover:-translate-y-0.5 hover:shadow-md ${isOverdue ? 'border-red-400 ring-2 ring-red-100' : 'hover:border-slate-300'}`} href={`/work/${order.id}`} key={order.id}>
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-2"><StatusBadge value={order.status} /><StatusBadge value={order.priority} />{isOverdue && <span className="rounded-full bg-red-600 px-2.5 py-1 text-xs font-semibold text-white">Po termínu</span>}<span className="text-sm font-medium text-slate-500">{workTypeLabels[order.workType]}</span></div>
                  <h3 className="mt-2 text-lg font-bold text-slate-950">{order.title}</h3>
                ...
                <div className="text-sm md:text-right"><p className="font-semibold text-slate-900">{formatWorkPrice(order.price?.toString())}</p><p className="font-medium text-slate-700">{order.assignments.map((assignment) => assignment.workerName).join(', ') || 'Nepřiřazený pracovník'}</p><p className="text-slate-500">Interní úkoly: {order.workTasks.length}</p>{order.quantity && <p className="text-slate-500">{order.quantity} ks {order.mediaLabel || ''}</p>}</div>
              </div>
            </Link>;
          })}
        </section>
