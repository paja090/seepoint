      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="space-y-3">
          <div className="flex items-end justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold">Nejbližší pracovní úkoly</h2>
              <p className="text-sm text-slate-500">Řazeno podle data provedení. Kompletní plán zůstává v týdenním rozpisu.</p>
            </div>
            <span className="text-sm font-medium text-slate-500">{listOrders.length} z {orders.length} úkolů</span>
          </div>

          {orders.length === 0 ? (
            <div className="card text-center">
              <p className="font-medium">Zatím zde není žádný pracovní úkol.</p>
              <p className="mt-1 text-sm text-slate-500">První úkol vytvořte ve formuláři výše. Původní tabulka zůstává beze změny.</p>
            </div>
          ) : (
            listOrders.map((order) => {
              const isOverdue = order.deadlineAt && order.deadlineAt < now && !['DONE', 'CANCELLED'].includes(order.status);
              return (
                <Link className={`card block transition hover:-translate-y-0.5 hover:shadow-md ${isOverdue ? 'border-red-400 ring-2 ring-red-100' : 'hover:border-sky-300'}`} href={`/work/${order.id}`} key={order.id}>
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${workStatusStyles[order.status]}`}>{workStatusLabels[order.status]}</span>
                        <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${workPriorityStyles[order.priority]}`}>{workPriorityLabels[order.priority]}</span>
                        {isOverdue && <span className="rounded-full bg-red-600 px-2.5 py-1 text-xs font-semibold text-white">Po termínu</span>}
                        <span className="text-sm font-medium text-slate-500">{workTypeLabels[order.workType]}</span>
                      </div>
                    ...
                    <div className="text-sm md:text-right">
                      <p className="font-semibold">{formatWorkPrice(order.price?.toString())}</p>
                      <p className="font-medium">{order.assignments.map((assignment) => assignment.workerName).join(', ') || 'Nepřiřazený pracovník'}</p>
                      <p className="text-slate-500">Interní úkoly: {order.workTasks.length}</p>
                      {order.quantity && <p className="text-slate-500">{order.quantity} ks {order.mediaLabel || ''}</p>}
                    </div>
                  </div>
                </Link>
              );
