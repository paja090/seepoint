...
import { formatWorkDate, formatWorkPrice, workTypeLabels } from '@/lib/work';
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <header className={`card ${isOverdue ? 'border-red-400 ring-2 ring-red-100' : ''}`}>
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div><div className="flex flex-wrap items-center gap-2"><StatusBadge value={order.status} /><StatusBadge value={order.priority} />{isOverdue && <span className="rounded-full bg-red-600 px-2.5 py-1 text-xs font-semibold text-white">Po termínu</span>}<span className="text-sm text-slate-500">{workTypeLabels[order.workType]}</span></div><h1 className="mt-3 text-3xl font-bold text-slate-950">{order.title}</h1><p className="mt-2 text-slate-600">{order.client?.name || order.clientName}</p><p className="mt-1 text-sm font-medium text-slate-500">Zadal/a: {order.requestedBy || 'Neuvedeno'}</p></div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <aside className="space-y-6">
            ...
            <section className="card"><h2 className="text-lg font-bold">Kontakt na místě</h2><p className="mt-2 text-sm">{order.contactName || 'Neuveden'}</p>{order.contactPhone && <a className="mt-1 block text-sm font-medium text-slate-700 hover:text-slate-950 hover:underline" href={`tel:${order.contactPhone}`}>{order.contactPhone}</a>}</section>
          </aside>
        </div>
        <WorkOrderEditForm
