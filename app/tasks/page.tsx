...
export default async function TasksPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const tasks = await prisma.workTask.findMany({ where, include: { assignedTo: true, createdBy: true, carrier: true, vehicle: true }, orderBy: [{ status: 'asc' }, { scheduledDate: 'asc' }, { dueDate: 'asc' }], take: 500 });

  return (
    <AppShell>
      <div className="mb-6"><h1 className="text-3xl font-bold">Úkoly</h1><p className="mt-1 text-sm text-slate-500">Provozní a technické úkoly napojené na zaměstnance, nosiče a vozidla.</p></div>
      ...
      <section className="card overflow-x-auto"><div className="mb-3 flex items-center justify-between"><h2 className="text-xl font-bold">Seznam úkolů</h2><p className="text-sm text-slate-500">Zobrazeno {tasks.length}</p></div>{tasks.length === 0 ? <p className="text-sm text-slate-500">Zatím není evidovaný žádný úkol.</p> : <table className="w-full min-w-[980px] text-left text-sm"><thead className="text-xs uppercase tracking-wide text-slate-500"><tr><th className="border-b py-2 pr-3">Úkol</th><th className="border-b py-2 pr-3">Pracovník</th><th className="border-b py-2 pr-3">Termín</th><th className="border-b py-2 pr-3">Nosič</th><th className="border-b py-2 pr-3">Vozidlo</th><th className="border-b py-2 pr-3">Priorita</th><th className="border-b py-2 pr-3">Stav</th></tr></thead><tbody>{tasks.map((task) => <tr className="border-b last:border-0" key={task.id}><td className="py-3 pr-3"><b>{task.title}</b><br /><span className="text-slate-500">{task.description ?? task.note ?? '-'}</span></td><td className="py-3 pr-3">{task.assignedTo ? `${task.assignedTo.firstName} ${task.assignedTo.lastName}` : '-'}</td><td className="py-3 pr-3">{dateOnly(task.scheduledDate)}<br /><span className="text-slate-500">do {dateOnly(task.dueDate)}</span></td><td className="py-3 pr-3">{task.carrier?.code ?? task.location ?? '-'}</td><td className="py-3 pr-3">{task.vehicle?.name ?? '-'}</td><td className="py-3 pr-3"><StatusPill value={task.priority} /></td><td className="py-3 pr-3"><StatusPill value={task.status} /></td></tr>)}</tbody></table> }</section>
    </AppShell>
  );
}
