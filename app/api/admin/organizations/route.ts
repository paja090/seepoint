...
export async function POST(request: Request) {
  try {
    ...
    const result = await platformPrisma.$transaction(async (tx) => {
      const organization = await tx.organization.create({ data: { name, slug, companyId: typeof body?.companyId === 'string' ? body.companyId.trim() || null : null, vatId: typeof body?.vatId === 'string' ? body.vatId.trim() || null : null, email: typeof body?.email === 'string' ? body.email.trim() || null : null, plan: 'START', subscriptionStatus: 'TRIAL', trialEndsAt: new Date(Date.now() + 14 * 86400000) } });
