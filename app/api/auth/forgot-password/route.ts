...
export async function POST(request: Request) {
  ...
  if (user?.status === 'ACTIVE') { try { const token = await issueUserToken(user.id, 'PASSWORD_RESET', 1); await sendPasswordResetEmail(user.email, `${process.env.APP_URL ?? 'http://localhost:3000'}/reset-password/${token}`); } catch (error) { console.error('[auth/forgot-password] Email delivery failed', error instanceof Error ? error.message : String(error)); } }
