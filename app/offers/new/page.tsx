...
export default async function NewOfferTypePage() {
  await requirePageAccess('offers');
  return <AppShell><PageHeader description="Každý typ má vlastní data a workflow. Navigace nejsou běžné reklamní plochy." title="Jaký typ nabídky vytváříte?" /><div className="grid gap-5 lg:grid-cols-3">{types.map(({ href, title, description, icon: Icon, tone }) => <Link className="group flex min-h-64 flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-slate-300 hover:shadow-lg" href={href} key={href}><span className={`grid size-14 place-items-center rounded-2xl ring-1 ${tone}`}><Icon size={27} /></span><h2 className="mt-6 text-xl font-semibold text-slate-950">{title}</h2><p className="mt-2 flex-1 text-sm leading-6 text-slate-600">{description}</p><span className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-slate-950">Pokračovat <ArrowRight className="transition group-hover:translate-x-1" size={16} /></span></Link>)}</div></AppShell>;
}
