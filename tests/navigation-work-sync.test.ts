...
test('syncNavigationOrderToWorkOrderInTransaction: creates WorkOrder and syncs WorkTasks when installation is planned', async () => {
  ...
  const woData = createdWorkOrderData as Record<string, unknown> | null;
  assert.ok(woData);
  assert.equal(woData.navigationOrderId, 'nav-2');
  assert.equal(woData.title, 'Montáž navigace: Form Factory s.r.o. – Form Factory Ostrava');
  assert.equal(woData.workType, 'INSTALLATION');
  assert.equal(woData.status, 'PLANNED');
  const createdAssignments = woData.assignments as { create: Array<{ workerName: string }> };
  assert.equal(createdAssignments.create[0].workerName, 'Pavel Montér');
  const createdItems = woData.items as { create: Array<unknown> };
